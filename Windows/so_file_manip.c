...
int so_fclose(SO_FILE *stream)
{
	...
	if (stream != NULL) {
		if (stream->buf_len > 0) {
			/* if there's anything to write, write before closing */
			r = so_fflush(stream);
			if (r < 0) {
				free(stream);
			return SO_EOF;
...
int so_fflush(SO_FILE *stream)
{
	...
	if (stream != NULL) {
		if (stream->prev_op == 'w') {
			/*
			 * flush only makes sense if the last operation was
			 * a write
			 */
			if (stream->mode[0] == 'a') {
			/*
			 * If the file is open in append mode, the cursor has
			 * to be moved at the end of the file. The previous
			 * cursor is still kept for another possible read in
			 * stream->cursor and won't be affected.
			 * At another read, the actual cursor will
			...
			while (stream->buf_len > 0) {
				bRet = WriteFile(
					...
					stream->buffer + stream->buf_cursor,
				...
				if (stream->buf_len > 0 && bRet != FALSE) {
					stream->buf_len -= dwBytesWritten;
					stream->buf_cursor += dwBytesWritten;
					stream->cursor += dwBytesWritten;
				} else {
					stream->error_flag = SO_EOF;
					return SO_EOF;
				}
...
int so_fseek(SO_FILE *stream, long offset, int whence)
{
	...
	if (stream != NULL) {
		if (stream->prev_op == 'r') {
			/*
			 * If the last operation was read,
			...
			if (stream->prev_op == 'w') {
				/*
				 * If the last operation was write,
		...
		/* Now we can put the cursor where we want to, then store it. */
...
long so_ftell(SO_FILE *stream)
{
	...
	if (stream != NULL) {
		if (stream->prev_op != 'r') {
			/*
			 * If the last operation was not read,
			 * do what fseek would do.
			 */
		...
		/*
		 * Otherwise, we have the current cursor stored
		 * and at the ready.
		 */
...
size_t so_fread(void *ptr, size_t size, size_t nmemb, SO_FILE *stream)
{
	...
	if (stream != NULL) {
		if (stream->prev_op == 'w') {
			/*
			 * If the last operation was a write and we have
			 * something in the buffer, we need to flush it
			 * to the file.
			 */
			if (stream->buf_len > 0) {
				r = so_fflush(stream);
	...
	for (i = 0; i < nmemb; i++) {
		for (j = 0; j < size; j++) {
			...
			if (stream->error_flag != SO_EOF)
				...
				/*
				 * If we got to EOF and a full sized element
				 * hasn't been read -> error
				 */
				if (j != size - 1)
					return 0;
				break;
...
size_t so_fwrite(const void *ptr, size_t size, size_t nmemb, SO_FILE *stream)
{
	...
	if (stream != NULL) {
		if (stream->prev_op == 'r') {
			/*
			 * If the last operation was a read, the buffer needs
			 * to be invalidated and the buffer positioned at the
			 * last read character
			 */
...
int so_fgetc(SO_FILE *stream)
{
	...
	if (stream != NULL) {
		...
		if (stream->buf_cursor < stream->buf_len) {
			/*
			 * The other read cases where we don't need to
